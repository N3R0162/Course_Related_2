...
def process_wflw(anno, target_size):
    ...
    lmk = anno[120:152]
    lms = [float(x) for x in lmk]
    lms_x = lms[0::2]
    lms_y = lms[1::2]
    # Make sure that the landmarks are in the picture by replace negative values with 0 and large value
    # that go beyond the size of the image by the size of the image:
    ...
    landmarks = lms.copy()
    lms = [x for z in lms for x in z] #Flatten [[x1,y1], [x2,y2], ...] to [x1,y1,x2,y2]
