...
def demo_image(image_path, net, preprocess, input_size, net_stride, num_nb, use_gpu, device):
    ...
    for image_file in os.listdir(image_path):
        ...
        detections = [[1, 1, 0, 0, image_width-1, image_height-1]]
        for i in range(len(detections)):
            ...
            print("LMS PRED: ", lms_pred_nb_x)
            print("lms_pred_nb_x[reverse_index1, reverse_index2]: ", lms_pred_nb_x[reverse_index1, reverse_index2])
            print("SHAPE: ", lms_pred_nb_x.shape)
            print("Reverse index 1: ", reverse_index1)
            print("Reverse index 2: ", reverse_index2)
        ...
        compare_image = cv2.hconcat([image_groundtrust, image])
        image_width, image_height, _ = compare_image.shape
        scale = 0.80
        image_height_new = int(image_height*scale)
        image_width_new = int(image_width*scale)
        resize_image = cv2.resize(compare_image, (image_height_new, image_width_new))
        cv2.imshow("Facial Landmarks", resize_image)
        while True:
            key = cv2.waitKey(1)    
